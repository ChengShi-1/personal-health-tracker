...
Deno.serve(async req=>{
  ...
  if(!authorization||!supabaseUrl||!anonKey)return json({error:'请先登录 Supabase'},401);
  ...
  try{
    const {message,history=[],today}=await req.json();if(typeof message!=='string'||!message.trim())return json({error:'请输入健康记录'},400);
    const input=[...history.slice(-8).map((x:any)=>({role:x.role==='assistant'?'assistant':'user',content:String(x.content).slice(0,2000)})),{role:'user',content:`今天是 ${today}。用户输入：${message}`}];
    ...
    const text=outputText(payload);if(!text)throw new Error('模型没有返回可解析内容');return json(JSON.parse(text));
